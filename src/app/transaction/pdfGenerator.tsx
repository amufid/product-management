   ...
   const MyDocument = () => (
      <Document>
         <Page size="A4" style={styles.page}>
            <Text style={styles.header}>Riwayat Transaksi {months.find((item) => item.id === month)?.name} {year}</Text>
            {/* Header Tabel */}
               ...
               <View style={styles.table}>
                  ...
                  {filteredData.map((data, i) => (
                     <View style={styles.tableRow} key={data.id}>
                        ...
                        <View style={styles.tableCol}>
                           <Text style={styles.tableCell}>{products.find((product) => product.id === data.productId)?.name}</Text>
                        </View>
                        <View style={styles.tableCol}>
                           <Text style={styles.tableCell}>{data.quantity}</Text>
                        </View>
                        <View style={styles.tableCol}>
                           <Text style={styles.tableCell}>{currencyFormat(data.totalPrice)}</Text>
                        </View>
                        <View style={styles.tableCol}>
                           <Text style={styles.tableCell}>{data.type}</Text>
                        </View>
                        <View style={styles.tableColDate}>
                           <Text style={styles.tableCell}>{convertDate(data.createdAt)}</Text>
                        </View>
                     </View>
                  ))}
               </View>
                           ...
                           <Select onValueChange={(value) => setMonth(value)}>
                              <SelectTrigger>
                                 <SelectValue placeholder='Pilih bulan'>
                                    {months.find((item) => item.id === month)?.name || 'Pilih bulan'}
                                 </SelectValue>
                              </SelectTrigger>
