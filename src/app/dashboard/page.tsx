...
export default async function Home() {
  ...
  const [products, categories, transactions, users] = await Promise.all([
    productResponse,
    categoryResponse,
    transactionResponse,
    userResponse,
    ...
    <main className="flex flex-col gap-3 m-5">
      ...
      <div className="flex flex-col sm:flex-row items-center w-full gap-3">
        <div className="w-full sm:w-[25%] h-40 bg-slate-300 dark:bg-slate-950 p-7 rounded-sm flex flex-row items-center justify-between border">
          ...
          <div>
            <CountUpAnimation initialValue={0} targetValue={products.length} />
            <h2>Total produk</h2>
          </div>
        </div>
        <div className="w-full sm:w-[25%] h-40 bg-slate-300 dark:bg-slate-950 p-7 rounded-sm flex flex-row items-center justify-between border">
        ...
        <div className="w-full sm:w-[25%] h-40 bg-slate-300 dark:bg-slate-950 p-7 rounded-sm flex flex-row items-center justify-between border">
          ...
          <div>
            <CountUpAnimation initialValue={0} targetValue={users.length} />
            <h2>Total user</h2>
          </div>
        </div>
      </div>
